#!/usr/bin/env python3
"""
add_German_labels.py
────────────────────
For every Q845945 instance that has an English label like "X Shrine" but no German label,
add a German label of the form "X-schrijn".

USAGE
─────
  python add_German_labels.py            # live
  python add_German_labels.py --dry      # preview only
  python add_German_labels.py --verbose  # chatty
"""
...
HEADERS = {
    "User-Agent": "ImmanuelleGermanLabelBot/0.1 (https://wikidata.org/wiki/User:Immanuelle; mailto:immanuelleleonhart@gmail.com)"
}
...
def make_nl_label(en_label):
    ...
    return f"{base} Schrein"

def add_German_label(qid, nl_label, verbose, dry):
    if dry and verbose:
    ...
    r = wd_post({
        ...
        "language": "de",
        "value": nl_label,
        "summary": "Bot: add German label from English"
    })
...
def query_items():
    query = """SELECT ?item ?enLabel WHERE {
      ...
      OPTIONAL { ?item rdfs:label ?nlLabel. FILTER(LANG(?nlLabel) = "de") }
...
def main(dry=False, verbose=False):
    ...
    for qid, en_label in items:
        ...
        result = add_German_label(qid, nl_label, verbose, dry)
        if result == "added":
            added += 1
        else:
            skipped += 1
    ...
    print(f"\n✅ {added} German label(s) {action}, {skipped} skipped.")
