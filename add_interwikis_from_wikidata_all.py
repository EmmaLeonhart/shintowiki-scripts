"""add_interwikis_from_wikidata_all.py
================================================
Add interwiki links from Wikidata to ALL pages (refresh mode).
...
4. Removes any existing interwiki block and adds fresh one at the top
================================================
"""
...
def get_wikidata_interwikis(qid):
    ...
    try:
        ...
        print(f"     ! Error querying Wikidata {qid}: {e}", flush=True)
        return []
...
def safe_save(page, text, summary):
    ...
    if not page.exists:
        print(f"   • skipped save, page [[{page.name}]] no longer exists", flush=True)
        return False
    ...
    try:
        ...
        if getattr(e, "code", "") == "editconflict":
            print(f"   ! edit conflict on [[{page.name}]] – skipping", flush=True)
            return False
        raise
        ...
        print(f"   ! Save failed on [[{page.name}]] – {e}", flush=True)
        return False


def main():
    """Add interwikis from Wikidata to ALL pages (refresh mode)."""

    print("Adding interwikis from Wikidata to ALL pages (refresh mode)", flush=True)
    print("=" * 60, flush=True)

    # Get the category
    category = site.pages['Category:Pages linked to Wikidata']

    print(f"\nFetching mainspace pages in [[Category:Pages linked to Wikidata]]...", flush=True)
    try:
        ...
        print(f"ERROR: Could not fetch category members – {e}", flush=True)
        return
    ...
    for idx, page in enumerate(members, 1):
        try:
            ...
            print(f"{idx}. [[{page_name}]] ({qid})", end="", flush=True)

            # Get interwikis from Wikidata
            interwikis = get_wikidata_interwikis(qid)

            if not interwikis:
                print(f" ... • No interwikis on Wikidata", flush=True)
                no_interwiki_count += 1
                continue

            # Build new interwiki string (just prepend, don't remove anything)
            interwiki_lines = "<!--interwikis from wikidata-->"
            for lang_code, page_title in interwikis:
                interwiki_lines += f"[[{lang_code}:{page_title}]]"
            interwiki_lines += "\n"

            # Prepend to beginning of page (keep everything else)
            new_text = interwiki_lines + text

            # Save the page
            if safe_save(page, new_text, "Bot: refresh interwikis from Wikidata"):
                updated_count += 1
                print(f" ... ✓ Updated with {len(interwikis)} interwikis", flush=True)
            else:
    ...
    print(f"\n{'=' * 60}", flush=True)
    print(f"\nSummary:", flush=True)
    print(f"  Total pages: {len(members)}", flush=True)
    print(f"  Processed: {processed_count}", flush=True)
    print(f"  Updated: {updated_count}", flush=True)
    print(f"  No interwikis on Wikidata: {no_interwiki_count}", flush=True)
