...
ATOMIC_FILES = [
    ...
    "orphan_description_removals.txt",        # STEP 1 of Emma's description algorithm (2026-08-21): clear any description on an item that has NO LABEL in that language. Her caveat, verbatim: "we always remove descriptions from items without a label in that language". Wikidata's uniqueness constraint is on the (label, description) PAIR -- either alone may repeat -- so a description on a label-less item stakes the half that matters least, and when the label finally arrives the completed pair can collide and the LABEL edit is what gets rejected. "Since labels are overwhelmingly more important than descriptions, it follows that any description on an item without a label is actively harmful." A description with no label costs a label. Lines are `Qxxx|D<lang>|""` -- an empty-value wbsetdescription, which CLEARS it; deliberately not the `-Qxxx|D..|"text"` dash form, which is value-matched (and which execute_line refuses outright). Generated by audit_orphan_descriptions.py. Emma authorised id + uk specifically, the two languages that are 94% of the 10,250 orphans. UNCAPPED on purpose: these unblock label edits, so draining them slowly is the cost the rule exists to avoid.
