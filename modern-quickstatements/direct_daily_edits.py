...
ATOMIC_FILES = [
    ...
    "p958_from_kokugakuin.txt",               # P958 sections READ OFF the Kokugakuin entry pages themselves, for the 228 items with no P1352 anywhere — the bucket where the section exists only on the page. Emma 2026-08-25: "grind it." generate_p958_from_kokugakuin.py + kokugakuin_candidates.py; the pages are committed under kokugakuin_pages/ so a value can be audited against the exact bytes it was read from. Matching is exact normalised equality against that ONE entry's 2-5 numbered candidates, never a global search, and ties defer — measured against 120 items with a known P958 at 74 answers, 74 correct (two of which corrected Wikidata). ADD-only and skips any item that already carries a section on that id. ⛔ A COLLISION GUARD drops any (id, section) claimed by two items: identity is the PAIR, two items share a ja label often enough to matter, and the live data currently has ZERO such collisions — 17 of 57 lines were dropped for this on the first run.
    "p958_by_entry.txt",                      # P958 sections assigned one ENTRY at a time against its full holder set, from the committed Kokugakuin corpus. Supersedes the per-item approach for correctness reasons rather than coverage: a section is an assignment WITHIN an entry, so resolving the whole entry makes collisions structurally impossible instead of something a post-hoc guard has to catch (the per-item reader had to drop 17 of 57 lines). Already-set sections act as constraints, which is what makes the MISSING-SOME bucket tractable. EXACT MATCH ONLY: held-out validation on 280 known sections put exact match at 142/142 and the 1x1 elimination rule at 3 right / 1 wrong, so elimination is OFF by default — it assumes every slot's occupant is among the holders, and hands orphan slots to whoever is left. generate_p958_by_entry.py. ADD-only.
