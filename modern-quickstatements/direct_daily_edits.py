...
ATOMIC_FILES = [
    ...
    "description_restores.txt",             # Restores 208 descriptions the description-fix drip downgraded on 2026-09-09/10, across uk/es/it/cs/vi/nl/ca. Each value comes from the parent revision of our own edit, so it is the exact string that was there, not a rebuild from P131. Emits a line only where the original named the item's prefecture and the current value does not, so an item a human has already repaired is skipped and the file empties itself as the restores land (generate_description_restores.py).
