...
ATOMIC_FILES = [
    "orphan_membership_removals.txt",         # Modern shrines carrying `part of` into the SAME list their own P460 register entry belongs to — the defect list_membership_removals.txt exists for, in the population it never selected. That file's 2,151 were chosen AS RONSHA, so it is narrower than the rule: CLAUDE.md's test is "check the item for P460 — if it points at a Q135…-era entry item, the item is a modern shrine and its P361 is the thing being removed." 2,788 items meet it, 2,008 are already staged there, and these 829 lines are the 816 that were not. Emma approved staging 2026-08-24. Value-matched, so an affected item loses ALL its membership into that list — intended, per "we remove it and then we add it again. This is very, very established." Generator generate_orphan_membership_removals.py. REMOVE-only; the re-add is the separate later job.
    "multi_ordinal_removals.txt",             # `part of` statements carrying MORE THAN ONE series ordinal — the piped-link import collapse. Emma 2026-08-25: "we remove them entirely and then later on we have quickstatements that add proper membership stuff for the lists", and "every single membership thing on those items should be removed unless the membership of the Shikinaisha list is 100% accurate". So a value-matched -Q|P361|Q takes EVERY membership statement on an affected item, which is intended, not a hazard — an item whose membership was folded together once is not trusted to have got the rest right. Splitting per-ordinal was rejected: P155/P156 are piled in too and pairing five ordinals to five predecessors is a guess, not a repair. Generator generate_multi_ordinal_removals.py. REMOVE-only; the re-add is a separate later job.
    ...
    "derived_name_in_kana.txt",             # The general case of the derived reading: every shrine with a ja label, an en label and no P1814. Same english_to_kana derivation as katakana_reading_add.txt, same rule behind it (CLAUDE.md: "the English label IS the KANA reading"), pointed at 16,753 items instead of four. Only the two tiers Emma cleared on 2026-09-10 are in the file — derivation agreeing with the item's name-mates (97.7% on a held-out set of shrines that have both an en label and a real reading), and items whose ja label has no name-mate reading at all (93.9%). The 1,348 where the name-mates contradict the derivation are routed to name_in_kana/ work-files instead, where the jawiki lead decides. ADD-only, self-healing (generate_derived_name_in_kana.py).
    "katakana_reading_add.txt",             # The residue the カミノヤシロ pipeline genuinely cannot reach. 27 top-level katakana P1814 statements sit on shrines with no ojp-hani P1448 of their own, but most are Shikinai Ronsha whose Engishiki ENTRY item carries the name one P460/P361 hop away — the reading on the candidate, the name on the entry — and those ARE the pipeline's, via its SEED branch (docs/script-rationale/katakana_name_in_kana_2026-09.md, 2026-09-10 correction). What is left is 4 items with no entry item behind them. Their hiragana is DERIVED by english_to_kana.kana_for from the English label plus the shrine suffix in the Japanese label — never read out of an article, per CLAUDE.md's "the English label IS the KANA reading" — and the generator refuses anything it cannot do confidently, which is what keeps it off 四至神/座摩神 (ruled correct as they stand), 岩井温泉 (not a shrine) and 一之宮神社 (スサノオ: a deity in a reading field, a wrong FIELD not a short reading). ADD-only, self-healing (generate_katakana_reading_add.py).
    ...
    "french_elision_fixes.txt",               # Emma 2026-08-04: "de Ō should be corrected to d'Ō across them with an additional pipeline thing that makes the quickstatements instantly if it is something that was universal". Measured first: de+true-vowel is 0 of 23,892 fr-labelled shrines (real vowels are already 100% elided), so the only live class is H — where the corpus itself has ruled 3,645 d’H to 25 de H, the same name appearing both ways. These are those 25. The generator stays general, so the first "de Ōmi…" a future label pass introduces is caught next run. shinto_miraheze/generate_french_elision_fixes.py.
    ...
    "description_label_pairs.txt",            # LABEL-ONLY since 2026-09-11. Emma: "descriptions should not be being edited either way really. The emergency stuff was intended to rapidly apply labels to things with orphaned descriptions to see how much actual description changes were needed." An orphan description — one in a language the item has no label in — costs the item a label, because the uniqueness constraint is on the (label, description) PAIR, so supplying the label IS the fix; 13,099 orphans remained on 2026-09-11. Bare Qxxx|Lxx|"…" lines. The compound Dxx||Lxx pair shape is GONE with the template inference that flattened 208 prefecture-specific descriptions into one duplicative string on 2026-09-10 — the description edit only ever existed to unblock the label, and the label lands without it: a colliding label is WITHHELD rather than the description rewritten to make room. Capped ~100/day below; regenerated in CI by generate_description_fixes.py
    "address_citation_from_article.txt",    # Reference-only backfill for P6375: S143=Q177837 + S4656 pointing at the item's own jawiki article, where that article's 所在地 states the same address. No value is changed — execute_line finds the claim by value and attaches the reference. 818 candidates were REFUSED because the article states a different address (鍛治屋 vs 鍛冶屋, a different block number, a different town): citing those would assert what the article does not say. Disjoint from uncited_address_removals.txt, which only acts on items that also carry a CITED address (generate_address_citation_from_article.py, 2026-09-11).
    "souken_p571_citations.txt",            # Reference-only backfill: S143=Q177837 + S4656 onto existing unreferenced P571 statements whose year still matches jawiki's 創建/創建年 field. No value is changed — execute_line finds the claim by value and attaches the reference group. Separate from souken_p571.txt so the backfill can be paced or stopped without touching the import (generate_souken_quickstatements.py, 2026-09-11).
    "saijin_named_as.txt",                  # P1932 onto EXISTING P825 deity statements — the source spelling jawiki gives, e.g. [[天照大神|天照皇大御神]] -> P1932 "天照皇大御神". Separate from saijin_deity_research.txt so the backfill can be paced or stopped without touching the import. A qualifier-bearing line makes execute_line find the existing claim and add to it rather than create a second one. clean_named still refuses a multi-name or parenthetical display, so a piped label listing three deities stores nothing rather than junk (generate_saijin_deity_research.py).
    "description_removals.txt",             # Clears every description off an item Emma has named. Q11558526 was a shrine carrying 40 "Wikimedia disambiguation page" descriptions and 38 orphan descriptions (a description in a language with no label, which docs/description_label_policy.md records as actively harmful because it costs a label); she hit the anti-abuse limiter removing them by hand and asked for the pipeline to do it. `Qxxx|Dyy|""` -- a term line with an empty value, the QS v1 removal form. The hyphenated codes in it (Dzh-hant, Dpt-br, Dtg-cyrl) are why parse_qs_line's term test moved from prop[1:].isalpha() to TERM_CODE. Self-healing, and zero-line whenever the named items are clean (generate_description_removals.py).
    ...
    "ronsha_role_qualifiers.txt",           # P2868=Q135022904 onto EXISTING P460 statements on pure Shikinai Ronsha items — qualifier-only, no value restated. The 2026-09-12 conformity re-measurement found this figure frozen at 1,613 of 2,058 across six weeks because NOTHING in this directory emitted P2868. The role is not a majority vote: 1,611 of the 1,613 already carrying it use the item's own P31 class, and the 2 using Q135026601 (Hiteisha) are a different claim. Excludes items also typed Q134917286 or Q135038714 — P31 is not exclusive and those are Engishiki ENTRIES, the same guard generate_ronsha_ojp_name_removals.py documents (generate_ronsha_role_qualifiers.py, 502 lines).
    "misplaced_honzon_form_removals.txt",    # REMOVE-ONLY, and the second half of a pair. An image FORM — 秘仏 hibutsu, 仏像, 涅槃仏, 磨崖仏 — reached P825 as a VALUE by the same parse damage as the designation above: the infobox writes |本尊 = [[阿弥陀如来]]（[[秘仏]]）. Emma 2026-09-13: "hibitsu and buddharupa are qualifiers", "qualifiers on the other thing" — so unlike the designation this is TRUE and gets MOVED, not deleted. generate_honzon_quickstatements now emits it as a P3831 role on the deity the field puts it after, and this removes the old value ONLY where a fresh SPARQL confirms that qualifier already landed. Empty until then, which is the design (generate_misplaced_form_removals.py). 4 statements on 3 items as of 2026-09-13.
    "invalid_p825_removals.txt",             # REMOVE-ONLY. P825 "dedicated to" pointing at Q1188622 (重要文化財, Important Cultural Property of Japan) — a designation, not a deity, so "dedicated to Important Cultural Property" claims nothing. Emma 2026-09-12: "a completely invalid thing and we should never add it and should universally remove it from all items it is present on." It arrived because generate_honzon_quickstatements took EVERY wikilink from the jawiki 本尊 field and the infoboxes write |本尊 = [[阿弥陀如来]]（[[重要文化財]]）; that generator now refuses it and 100 staged lines were stripped. Re-derived from live Wikidata each run, so it empties and goes inert (generate_invalid_p825_removals.py).
    ...
    "religious_building_p825.txt",           # P825 (dedicated to) for the religious-building corpus, from the dedication its LABEL already names. Emma 2026-09-18: "using the dedicated to for other ontology not just labels". The morpheme tables have resolved a label to a dedicatee since 2026-09-17, but only ever to a STRING — 安德肋 is not a value a statement can carry, Q43399 is. `match_dedication` names which table entry matched and `saint_qids.qid_for_match` validates the QID against ALLOWED_CLASSES; the 101-row seed those grew from was 52% wrong (French communes, a 1984 film, a Sufjan Stevens album, eight church buildings), so anything outside the checked map is refused rather than looked up at emit time. A dedication naming TWO saints is skipped, not halved — emitting one of them asserts the label names a single dedicatee. Generator shinto-label-generator/generate_religious_building_p825.py, regenerated every run. ADD-only.
...
# hyphenated subtags (zh-hant, pt-br, tg-cyrl, be-tarask, nan-hani).
TERM_CODE = re.compile(r"^[LDA][A-Za-z]{2,8}(?:-[A-Za-z0-9]{2,8})*$")
...
def parse_qs_line(line):
    ...
    #
    # The code test was `prop[1:].isalpha()`, which is every code WITHOUT a
    # hyphen — so `Dzh-hant`, `Dpt-br`, `Dtg-cyrl`, `Dbe-tarask` all failed it and
    # fell through to the claim path, where they were treated as a property named
    # "Dzh-hant" and refused. Hyphenated codes are a third of the description
    # languages on a bot-imported item (14 of 41 on Q11558526), so this was not a
    # rare edge. A property token is always P-prefixed and a reference S-prefixed,
    # so nothing else can match an L/D/A prefix.
    if TERM_CODE.match(prop):
        return {
            "entity": entity,
...
# no contact until we understand the editor." Source: docs/script-rationale/bruno_plus_analysis_2026-07.md.
#
