...
ATOMIC_FILES = [
    ...
    "emma_inbound_links.txt",                 # Emma 2026-07-16: "I need more stuff to link to me. At the moment not much stuff does link to me... even just my own papers and such linking to me... can help avoid me being deleted". Her papers ALREADY link to her — at Q140568964, the PRE-MERGE REDIRECT. WDQS does not traverse a redirect in statement position, so `?w wdt:P50 wd:Q140568870` returns ZERO and every notability query (including anything run at RfD) sees an author item with no papers. A merge artifact, not a missing edit. These 2 lines retarget P50 onto the live item, ordinals preserved, taking her from 0 inbound papers to 2. ADD-ONLY: the stale redirect-pointing statement is removed by a LATER pass once this add is confirmed (CLAUDE.md: never add+remove in one batch — random order could fire the remove first and lose the link). Static snapshot; safe to re-run because execute_line refuses claims that exist. Source: wikidata-review/emma-inbound-links.txt.
