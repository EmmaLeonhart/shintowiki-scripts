...
def render(cats, total):
    ...
    out = [f"# Stuck katakana `P1814` readings — {today}",
           "",
           "Generated by `modern-quickstatements/report_stuck_katakana_readings.py`. "
           "Report only; nothing is staged.",
           "",
           "## The ruling — these are being MOVED, and this report is now a burn-down",
           "",
           "Emma, 2026-09-13, given the choice between relocating these readings onto "
           "the ojp-hani official name and deleting them: *\"Move it, then delete.\"*",
           "",
           "`generate_kana_qualifier_add.py`'s second pass was widened that day to do "
           "it — it had been gated on the name carrying no `P1814` qualifier at all, "
           "which is the whole reason anything here was stuck. Every row below now has "
           "`<reading>カミノヤシロ` staged onto its name in `kana_qualifier_add.txt`, and "
           "`generate_kana_qualifier_remove.py` retires the top-level once a fresh "
           "SPARQL query confirms that exact value landed. **So the numbers here should "
           "fall on their own**; a category that stops shrinking is the thing to look "
           "at, not a category that is large.",
           "",
