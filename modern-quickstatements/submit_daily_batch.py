...
ATOMIC_FILES = [
    ...
    "description_restores.txt",               # Puts back the descriptions the description-fix drip replaced with a LESS specific one. Emma, 2026-09-10: "we're actively worsening Ukrainian descriptions ... turning descriptive ones into generic highly duplicative ones." generate_description_fixes inferred no prefecture template for nine languages -- the detection was a case-sensitive substring test against the full nominative label, which cannot match an inflected "у префектурі Наґано" or a lowercased "prefectuur Nagano" -- so every target in those languages fell to the generic modal and overwrote whatever was there. The original is read from each item's own revision history, never reconstructed. Self-healing (generate_description_restores.py).
