...
def main():
    # Rebound HERE, not at module scope: doing it on import replaces a
    # caller's stdout (pytest's captured stream, for one) with a wrapper around
    # a buffer it does not own.
    ...
    with open(_uos.path.join(HERE, "migrate_soja_add.txt"), "w", encoding="utf-8") as f:
        for l in add_lines:
            f.write(l + "\n")
    print(f"Wrote {len(add_lines)} lines to migrate_soja_add.txt")

    remove_lines = [f"-{iid}|{SRC_PROP}|{VALUE}" for iid in items_safe_remove]
    with open(_uos.path.join(HERE, "migrate_soja_remove.txt"), "w", encoding="utf-8") as f:
