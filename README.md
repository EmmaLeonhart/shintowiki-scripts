...
**▶ [ALL.txt](https://github.com/EmmaLeonhart/shintowiki-scripts/raw/main/_site/emergency-batch/ALL.txt)** — **10,000** lines drawn at random from the ~129,000 in the corpus, sized to one QuickStatements paste. Rebuilt by `generate-pages.yml` on every run, so each build deals a different hand and the file tracks the atomic files rather than lagging them; the [chunked files](_site/emergency-batch/) still hold every line.
...
**Run order is deliberate — in the chunks.** `00-orphan-labels.*.txt` runs first: labels for shrines that carry a *description with no label* in that language, each preceded by its own description-standardization line where one exists. Wikidata's uniqueness constraint is on the **(label, description) pair**, so a description with no label stakes the half that matters least, and when a label finally arrives the completed pair can collide — and it is the *label* edit that gets rejected. A description with no label costs a label. The standing remedy (`audit_orphan_descriptions.py`) therefore **deletes** the description; that is right only where no label exists, and where `shinto-label-generator/` has already generated one the deletion throws away a description we can complete instead. Hence labels first. The split between the two — fixable with a label vs. **removal residue** with no label to be had — moves every time labels get applied, and the live count is on the [orphan detail page](https://emmaleonhart.github.io/shintowiki-scripts/orphan-label-fixes.html); it is not restated here, because a number in a README does not get requeried. `ALL.txt` samples across the whole corpus uniformly, so it carries about 8% of the orphan labels rather than leading with all of them; work the `00-orphan-labels` chunks directly if that ordering is what you want.

Nothing in the batch deletes a description. `generate-pages.yml` rebuilds it (daily at 07:23 UTC and as the tail of every cleanup-loop, right after the QuickStatements are regenerated); `python site/generate_emergency_batch.py` rebuilds it by hand.
