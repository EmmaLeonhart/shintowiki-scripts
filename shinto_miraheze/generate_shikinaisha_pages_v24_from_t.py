#!/usr/bin/env python3
"""generate_shikinaisha_pages_v24_from_t.py
================================================
V24: Individual page edits approach, continuing from Kashima Shrine (Q135069590)
Uses the exact same page generation logic as v24_individual_edits.py
Skips all pages alphabetically before Kashima Shrine (Q135069590), then edits continuously
"""
...
def format_page_content(page_name, qid, entity):
    ...
    final_content = "<!--generated by generate_shikinaisha_pages_v24_from_t.py-->\n" + "\n".join(content_parts)
    return final_content

# ═══ MAIN PROCESS ═══

def main():
    print("Generating standardized Shikinaisha pages (V24 - Continuing from Kashima Shrine (Q135069590))\n")
    print("=" * 60)
    ...
    for i, page in enumerate(members):
        if page.name == START_PAGE:
            start_idx = i
            print(f"Found start page at index {i}: {START_PAGE}\n")
            break

    members = members[start_idx:]
    print(f"Processing {len(members)} pages starting from {START_PAGE}\n")
    ...
    for i, page in enumerate(members, 1):
        ...
        try:
            ...
            # Edit the page
            page.edit(new_content, summary="v24 from t: Standardize page format (continuing from Kashima Shrine (Q135069590))")
            print(f" ... ✓ Edited")
            processed_count += 1
