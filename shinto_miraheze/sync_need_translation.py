...
def main():
    ...
    for title, (wiki_revid, wiki_text) in wiki_pages.items():
        ...
        # Both changed → conflict. POLICY: need_translation is cloud-queue
        # "keyframe" work where the wiki is the authoritative source of truth
        # (same as duplicated_content), so the wiki ALWAYS wins — pull,
        # overwriting the local copy. Repo-only edits still push (handled
        # above); only genuine both-sides conflicts resolve to the wiki here.
        conflicts += 1
        if not args.apply:
