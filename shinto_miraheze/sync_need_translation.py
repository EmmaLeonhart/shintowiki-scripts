    ...
    for title in orphans:
        ...
        if base_sha is None:
            ...
            try:
                ...
                if page.exists:
                    summary = (
                        f"Sync from repo need_translation/ (re-add to category, "
                        f"page exists but was not in [[Category:Need translation]]) "
                        f"{args.run_tag}"
                    )
                else:
                    summary = (
                        f"Sync from repo need_translation/ (create page from repo) "
                        f"{args.run_tag}"
                    )
