#!/usr/bin/env python3
"""
update_shikinaisha_lists.py  •  revived 2026-07-04

Regenerates the "List of Shikinaisha in X Province" pages on
shinto.miraheze.org from Wikidata (province item → P527 shrine entries →
District/Name/Funding/Rank/Candidates/Notes/Address/Co-ords/DB table +
Shikigeisha table). These pages were ALWAYS generated — this is the revival
of archive/engishiki_list_generator/update_shikinaisha_lists_v3.py (archived
in f496f0f5, deleted with archive/ in 37fe5391), which stopped running; its
progress file additionally marked every page done PERMANENTLY, which is why
the pages went stale (Emma, 2026-07-04: they were never hand-authored, just
not properly updated).

Revival changes (2026-07-04):
  • NEW Address column from P6375 (prefer @ja) — the 同上-corrected,
    citation-backfilled addresses (queue.md 同上 rung).
  • House flags: --apply (default is dry preview), --max-edits, --run-tag,
    plus --pages "Title1;Title2" for targeted regeneration.
  • Credentials from WIKI_USERNAME/WIKI_PASSWORD env (bot-password format),
    no hardcoded fallback; login retries capped at 5.
  • --pages runs ignore the progress file (progress only resumes an
    interrupted full sweep; delete shiki_list_progress.json to restart one).
"""
...
# Bot-password creds from the environment, same as every other wiki script
# here (cleanup-loop.yml provides them). No hardcoded fallback.
USER = os.getenv("WIKI_USERNAME") or ""
...
def wiki_login() -> str:
    ...
    if not USER or not PASS:
        sys.exit("WIKI_USERNAME / WIKI_PASSWORD not set — refusing to run.")
    attempt = 0
    while attempt < 5:  # bounded — a wedged login must fail loudly, not sleep forever
        attempt += 1
        try:
    ...
    sys.exit("Login failed after 5 attempts — aborting.")
...
def address_cell(qid: str) -> str:
    """P6375 located-at-street-address, preferring the @ja value (the drip's
    同上-corrected import). Skips the literal 同上 shorthand — a still-broken
    claim must not resurface on the page it was imported from."""
    ent = get_entity_cached(qid)
    ja_val = other_val = None
    for cl in ent["claims"].get("P6375", []):
        ...
        if text == "同上":
            continue
        if dv.get("language") == "ja" and ja_val is None:
            ja_val = text
        elif other_val is None:
            other_val = text
    addr = ja_val or other_val
    return f"{{{{lang|ja|{addr}}}}}" if ja_val else (addr or '—')
...
def build_shiki_table(rows, token=None, dry=False, province=""):
    ...
    hdr = ('{| class="wikitable sortable"\n'
           ...
           '!! Rank !! Candidates !! Notes !! Address !! Co-ords !! Shrine&nbsp;DB')
    ...
    for _, prov, glob, name, kana, rank, q in rows:
        ...
        if not candidates:
            # Firmly identified — single row, notes/address/coords/DB from the entry itself
            ...
            lines += [
                ...
                cell(notes), cell(addr), cell(coords), cell(dbcell)
            ]
        else:
            ...
            for i, cand_q in enumerate(candidates):
                ...
                if i == 0:
                    ...
                    lines += [
                        ...
                        cell(cand_link),
                        cell(cand_notes), cell(cand_addr), cell(cand_coords), cell(dbcell)
                    ]
                else:
...
def process(title, token, dry, run_tag=""):
    src = wiki_get(title)
    ...
    summary = "Bot: Regenerate Shikinaisha list from Wikidata (incl. Address column)"
    if run_tag:
        summary += f" {run_tag}"
    wiki_edit(title, text, summary, token, dry)

# ────────────────────────────────────────────────────────
#  CLI
# ────────────────────────────────────────────────────────
if __name__ == "__main__":
    # In the main guard, not at import: tests import this module under pytest capture.
    sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')
    ap = argparse.ArgumentParser(description="Regenerate List-of-Shikinaisha province pages from Wikidata.")
    ap.add_argument("--apply", action="store_true",
                    help="actually save edits (default: dry preview)")
    ap.add_argument("--max-edits", type=int, default=100,
                    help="stop after this many page saves")
    ap.add_argument("--run-tag", default="",
                    help="tag appended to edit summaries")
    ap.add_argument("--pages", default="",
                    help='semicolon-separated exact page titles; overrides the full prefix walk and ignores the progress file')
    args = ap.parse_args()
    dry = not args.apply
    ...
    if args.pages:
        all_pages = [p.strip() for p in args.pages.split(";") if p.strip()]
        use_progress = False
        print(f"[INFO] Targeted run: {len(all_pages)} page(s)")
    else:
    ...
    for page in all_pages:
        ...
        if edited >= args.max_edits:
            print(f"[INFO] Reached max edits ({args.max_edits}); stopping.")
            break

        try:
            process(page, csrf, dry, run_tag=args.run_tag)
            edited += 1
            if use_progress:
                progress[page] = True
