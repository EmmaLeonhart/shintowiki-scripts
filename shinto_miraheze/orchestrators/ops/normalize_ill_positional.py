...
============================
For each ``{{ill|...}}`` call on a page, if there's an explicit ``|1=X``
named override, promote its value to be the bare first positional
parameter (replacing whatever was there) and drop the ``|1=`` entry.
...
Before::
    ...
    {{ill|Sukunahikona-no-Mikoto|ja|少彦名命|1=Sukunabikona|12=simple|13=User:Immanuelle/Sukunahikona-no-Mikoto|13=User:Immanuelle/Sukunabikona|ja_comment=jawiki redirects to スクナビコナ}}

After::

    {{ill|Sukunabikona|ja|少彦名命|12=simple|13=User:Immanuelle/Sukunahikona-no-Mikoto|13=User:Immanuelle/Sukunabikona|ja_comment=jawiki redirects to スクナビコナ}}
...
def _normalize_ill_body(body: str) -> str:
    ...
    for i, p in enumerate(parts):
        # Only consider parts that look like `key=value` with key == "1".
        # Don't strip the value — preserve the user's whitespace.
        if "=" not in p:
            continue
        k, v = p.split("=", 1)
        if k.strip() == "1":
