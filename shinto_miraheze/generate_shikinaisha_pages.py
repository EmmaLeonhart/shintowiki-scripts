#!/usr/bin/env python3
"""generate_shikinaisha_pages.py
================================================
Generate standardized shrine pages for Wikidata-generated Shikinaisha entries
================================================

This script:
1. Walks through [[Category:Wikidata generated shikinaisha pages]]
2. For each page with a {{wikidata link|QID}}, fetches Wikidata properties
3. Generates standardized page content with:
   - Bold title with shrine description
   - Sections for each property (P31, P625, P571, etc.)
...
def format_wikidata_link(entity, qid):
    """Format a wikidata link using ILL syntax with WD and LT parameters.

    Priority for first parameter:
    1. Shinto wiki page (if exists)
    2. English Wikipedia page
    3. English label
    """
    sitelinks = get_sitelinks(entity)

    # Try to find shinto wiki page (shintoism.miraheze.org)
    first_link = None
    ...
    # Add WD (Wikidata QID) and LT (Label)
    label = get_label(entity, 'en') or qid
    wd_link += f"|WD={qid}|LT={label}}}}}"
...
def get_property_heading(property_id):
    ...
    First tries to get label from Wikidata property, falls back to known labels.
    """
...
def format_claim_value(claim, entity):
    ...
    if isinstance(value, dict) and 'id' in value:
        qid = value['id']
        # Fetch label for the QID
        ref_entity = get_wikidata_entity(qid)
        ref_label = get_label(ref_entity, 'en') or qid
...
def format_page_content(page_name, qid, entity):
    ...
    intro = f"'''{page_name}''' is a {{{{nihongo|shrine|神社}}}}"

    # Get located in (P131) for location
    p131_claims = entity.get('claims', {}).get('P131', [])
    if p131_claims:
        located_qid = p131_claims[0].get('mainsnak', {}).get('datavalue', {}).get('value', {}).get('id')
        if located_qid:
            located_entity = get_wikidata_entity(located_qid)
            located_label = get_label(located_entity, 'en')
            if located_label:
                intro += f" located in {located_label}"

    intro += " in the {{{{ill|Engishiki Jinmyōchō|zh|延喜式神名帳|ja|延喜式神名帳}}}}."

    content_parts.append(intro)
    content_parts.append("")

    # Add Infobox BEFORE any text
    ...
    if deity and isinstance(deity, dict):
        ...
        if deity_qid:
            infobox_parts.append(f"| deity = {deity_qid}")
    ...
    # Now add sections for ALL properties
    all_claims = get_all_property_claims(entity)

    # Process properties in a standard order
    ...
    for prop_id in property_order:
        if prop_id in all_claims:
            ...
            for claim in claims:
                formatted_value = format_claim_value(claim, entity)
                if formatted_value:
    ...
    for prop_id in sorted(all_claims.keys()):
        if prop_id not in property_order:
            claims = all_claims[prop_id]
            prop_heading = get_property_heading(prop_id)
            content_parts.append(f"== {prop_heading} ==")
            content_parts.append("")
    ...
    # Add category from P361 (part of list - like "Shikinaisha in Izu Province")
    ...
    for claim in p361_claims:
        ...
        if isinstance(datavalue.get('value'), dict):
            ...
            if list_qid:
                ...
                if list_label:
                    categories.append(f"[[Category:{list_label}]]")
...
def main():
    ...
    print("Generating standardized Shikinaisha pages\n")
    print("=" * 60)
    ...
    for idx, page in enumerate(members, 1):
        try:
            ...
            # Save the page
            try:
                page.edit(new_content, summary="Bot: Generate standardized Shikinaisha page (upgraded script - full property sections, ILL links, P361 categorization)")
                processed_count += 1
                print(f" ... ✓ Updated")
