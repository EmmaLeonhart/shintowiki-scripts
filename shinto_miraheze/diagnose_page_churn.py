...
def _scan_category(category: str, sample_size: int, rev_limit: int):
    """Walk one category, sample pages, fetch revisions, attribute,
    detect alternation. Returns a per-category result dict."""
    print(f"\n=== {category} ===")
    print(f"Listing [[Category:{category}]] members...")
    all_titles = list_category_members(category)
    print(f"  found {len(all_titles)} mainspace pages")

    if len(all_titles) > sample_size:
        sample = random.sample(all_titles, sample_size)
    else:
        sample = list(all_titles)
    sample.sort()
    print(f"Sampling {len(sample)} pages, pulling last "
          f"{rev_limit} revisions each...")
    ...
    for i, title in enumerate(sample, 1):
        revs = fetch_revisions(title, rev_limit)
        scripts = [_attribute(r.get("user", ""), r.get("comment", ""))
                   for r in revs]
    ...
    return {
        "category": category,
        "total_members": len(all_titles),
        "sample_size": len(sample),
        "per_page": per_page,
        "alternation_hits": alternation_hits,
        "pair_histogram": pair_histogram,
        "script_total": script_total,
    }


def _format_category_section(result: dict, rev_limit: int) -> list[str]:
    """Markdown lines for a single category's findings."""
    out: list[str] = []
    cat = result["category"]
    out.append(f"## `[[Category:{cat}]]`")
    out.append("")
    out.append(
        f"Sample: {result['sample_size']} of {result['total_members']} pages. "
        f"Last {rev_limit} revisions per page."
    )
    out.append("")
    out.append("### Alternation summary")
    out.append("")
    hits = result["alternation_hits"]
    if not hits:
        out.append(
        ...
        out.append(
            f"{len(hits)} of {result['sample_size']} sampled pages "
            "show a two-script alternation streak (≥3 most-recent "
            "consecutive A→B→A→B revisions where neither side is unknown / human)."
        )
        out.append("")
        out.append("**Script-pair frequency:**")
        out.append("")
        for (a, b), n in result["pair_histogram"].most_common():
            out.append(f"* `{a}` ↔ `{b}`: **{n} page(s)**")
    out.append("")
    out.append("### Script-attribution counts (this category)")
    out.append("")
    for s, n in result["script_total"].most_common():
        out.append(f"* `{s}`: {n}")
    out.append("")
    out.append("<details><summary>Per-page edit traces (expand)</summary>")
    out.append("")
    for title, revs, scripts in result["per_page"]:
        alt = detect_alternation(scripts)
        marker = ""
        if alt:
            a, b, n = alt
...
def main() -> int:
    ...
    parser.add_argument("--category", action="append", default=None,
                        help="category to scan; pass multiple times to "
                             "scan several (default: just Git synced pages)")
    ...
    categories = args.category or [CATEGORY]

    results = [
        _scan_category(c, args.sample_size, args.rev_limit)
        for c in categories
    ]

    # Build markdown report
    out: list[str] = []
    out.append("# Page-churn diagnostic")
    out.append("")
    out.append(
        f"Generated by `shinto_miraheze/diagnose_page_churn.py`. "
        f"Scanned {len(categories)} categor"
        f"{'y' if len(categories) == 1 else 'ies'}; "
        f"sample size: {args.sample_size} per category; "
    ...
    # Cross-category top-line
    total_hits = sum(len(r["alternation_hits"]) for r in results)
    total_sampled = sum(r["sample_size"] for r in results)
    out.append("## Overall")
    out.append("")
    if total_hits == 0:
        out.append(
            f"**No alternation streaks detected** across "
            f"{total_sampled} pages sampled from "
            f"{len(categories)} categor"
            f"{'y' if len(categories) == 1 else 'ies'}."
        )
    else:
        out.append(
            f"**{total_hits}** alternation streaks across "
            f"{total_sampled} pages sampled from "
            f"{len(categories)} categories. See per-category sections."
        )
    out.append("")

    for r in results:
        out.extend(_format_category_section(r, args.rev_limit))

    text = "\n".join(out) + "\n"
    with open(args.output, "w", encoding="utf-8", newline="") as f:
        f.write(text)
    print(f"\nWrote {args.output} ({len(text)} chars)")
    print(f"Total alternation pages: {total_hits} / {total_sampled}")
    return 0
