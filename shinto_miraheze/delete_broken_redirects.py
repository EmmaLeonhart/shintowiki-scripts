#!/usr/bin/env python3
"""
delete_broken_redirects.py
===========================
Deletes pages listed on Special:BrokenRedirects — redirects that point
...
def iter_broken_redirects(site):
    """Yield page titles from Special:BrokenRedirects."""
    params = {
        "list": "querypage",
...
def main():
    ...
    parser.add_argument("--max-deletes", type=int, default=100,
                        help="Max deletions for this run (default 100).")
    parser.add_argument("--run-tag", required=True,
                        help="Wiki-formatted run tag link for delete summaries.")
    parser.add_argument("--dry-run", action="store_true",
                        help="Do not delete; only report actions.")
    args = parser.parse_args()

    site = mwclient.Site(
        WIKI_URL,
        path=WIKI_PATH,
        clients_useragent="BrokenRedirectDeleteBot/1.0 (User:EmmaBot; shinto.miraheze.org)",
    )
    site.login(USERNAME, PASSWORD)
    print(f"Logged in as {USERNAME}\n")

    checked = deleted = skipped = errors = 0
    for title in iter_broken_redirects(site):
        ...
        if not page.exists:
            print(f"{prefix} SKIP (already deleted)")
            skipped += 1
            continue
        ...
        try:
            page.delete(
                reason=f"Bot: delete broken redirect (target does not exist) {args.run_tag}"
            )
            deleted += 1
