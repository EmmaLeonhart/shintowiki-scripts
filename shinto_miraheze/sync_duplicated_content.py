...
==========================
Bidirectional sync between [[Category:Pages with duplicated content]] on
shinto.miraheze.org and the local ``duplicated_content/`` directory.
...
wiki (removing the category there too) and the local file is then deleted.

State lives in ``sync_duplicated_content.state`` (JSON). Default mode is
            ...
            try:
                ...
                if page.exists:
                    ...
                    if base_sha is None or local_sha != base_sha:
                        page.save(
                            local_text,
                            summary=f"Sync from repo: removed from Pages with duplicated content category {args.run_tag}",
                        )
                        edits_performed += 1
                        pushed += 1
                        time.sleep(THROTTLE)
