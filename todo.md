...
- [ ] **Shared login-retry helper.** Every wiki script does a single un-retried `site.login()`; a transient miraheze `LoginError` (spurious "credentials could not be authenticated") fails that step and its whole CI job — observed failing cleanup-loop run `27036877968` (2026-06-05) at `delete_lowercase_template_collisions.py`, which got a targeted `_login_with_retry` on 2026-06-06. Promote that to a shared helper (e.g. in `orchestrators/common.py` or a small `wiki_login.py`) and adopt it across the standalone scripts so one auth flake never red-marks a cleanup job again.
