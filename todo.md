...
- [ ] **Audit category pages for race-condition artifacts** — `resolve_category_wikidata_from_interwiki.py` and `create_category_qid_redirects.py` scripts ran concurrently in the past and may have left some categories in inconsistent state. Both scripts are no longer wired into any active workflow (verified 2026-05-08), and the `normalize_category_page` op now sweeps category pages continuously, so any actual artifacts are slowly being normalized. No clear failure mode to write an audit against — keep this as a watch item: if specific anomalies surface, write a targeted fix; otherwise let the orchestrator's category sweep handle it.
