...
- [ ] Task to do only after exhausting the 2026-08-04 tasks: We need to do a thing where we can add the wikidata template to existing articles. Look at https://shinto.miraheze.org/wiki/Shizensha it should get at the bottom a link to the wikidata item https://www.wikidata.org/wiki/Q139921367 remembering that our automated thing starts with the {{wikidata link}} template at the bottom of the page and moves from it into eventually actual wikidata links
- [ ] Task to do only after exhausting the 2026-08-04 tasks: We need to do a serious analysis of whether it will be a good idea to long term switch our operations to the fandom wiki. Fandom wiki has numerous disadvantages like a more unclear policy around bots, and it is risking something a bit more definitively useful, since the fandom wiki clearly preserves all of the history. But the fandom wiki editing actually appears to work. In this case the miraheze wiki would become a mirror that gets the fandom edits propagated to it eventually. Do not do this without my permission do the research and run an AskUserQuestion about it after
