...
- [ ] **Shared login-retry helper — finish the rollout.** Shipped `shinto_miraheze/wiki_login.py` (`login_with_retry`) 2026-06-06 and adopted it in the 6 cleanup-job deletion scripts (`delete_lowercase_template_collisions`, `delete_unused_templates`, `delete_unused_redirects`, `delete_unused_categories`, `delete_orphaned_talk_pages`, `delete_broken_redirects`) + this session's 3 new scripts (`report_double_qid_tail`, `report_multiple_wikidata_links`, `fix_ill_destinations`). **Remaining:** adopt it across the other ~25 standalone wiki-writing scripts wired into the workflows (each still does a single un-retried `site.login()`), so one transient miraheze auth flake never red-marks a CI job. Mechanical per-script swap (`site.login(U,P)` → `login_with_retry(site, U, P)` + `from wiki_login import login_with_retry`); syntax-check each.
