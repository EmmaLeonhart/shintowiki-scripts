...
- [ ] **Fandom vs Miraheze — the research is DONE, the decision is Emma's.** Written up 2026-09-12 in
  `docs/fandom_vs_miraheze_2026-09-12.md` and put to her by AskUserQuestion; she did not pick an
  option, answering instead *"the miraheze wiki is supposed to just get edited like normal so idk
  what is going on with it lol"* — which redirected the work to diagnosing the 403 (in `queue.md`).
  What still needs HER and nothing else: whether to reverse `FANDOM_SUNSET_DATE = 2027-01-01`
  (seven write entry points honour it, set 2026-05-13, two months before the first Miraheze 403).
  Do not migrate anything without her word.
