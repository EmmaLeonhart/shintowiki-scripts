...
- [ ] **Shared login-retry helper — orchestrator + fandom follow-up.** Shipped `shinto_miraheze/wiki_login.py` (`login_with_retry`) 2026-06-06. **Standalone rollout COMPLETE (2026-06-06 1pm work-loop):** all 62 standalone `shinto_miraheze/*.py` scripts that did a raw `site.login(...)` now use `login_with_retry` (in addition to the earlier 9 deletion + this-session scripts). Verified: `py_compile` every changed file + full 30-test suite green + no cross-imports (all run only as `__main__`, so the bare `from wiki_login import` sibling resolution is safe). **Remaining (separate — these are NOT standalone scripts in `shinto_miraheze/`, so a bare `import wiki_login` does NOT resolve; each needs a path-aware import):** (a) `shinto_miraheze/orchestrators/common.py` — the shared login for ALL 12 orchestrators, highest single-point value; (b) `shinto_miraheze/orchestrators/ops/fandom_mirror.py`; (c) `fandom/import_commons_wantedfiles_to_fandom.py`, `fandom/import_template_list_to_fandom.py`. For (a)/(b) add `sys.path`-aware import of `shinto_miraheze/wiki_login.py`; for (c) the helper lives in a different top-level dir, so either relocate/duplicate the helper or add a path shim.
