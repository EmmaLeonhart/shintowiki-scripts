...
- [ ] **QID-overlap merges — GO (Emma 2026-06-08: "combine and redirect, Jingū always wins").** Both sides are substantial real articles (dup-content cloud-queue is within-page only, doesn't fit), so these are per-pair editorial merges — do ONE carefully per tick, reviewable, never blind. Status:
  ...
  - **TRANSLATION-MERGES — need Emma's call (finding 2026-06-08).** These are NOT simple dups: one side is a clean English article, the OTHER is a raw/partial untranslated **Japanese** source with real detail the English lacks. `无邪志国造` (16k, raw JP: 概要/祖先/氏族/墓/系譜/考証…) ↔ `Musashi no Kuni no Miyatsuko` (7k English translation, has QID Q11504612); `Kehi Shrine` (100k = English stub + `{{Expand Japanese}}` + full raw JP 概要→文化財 + WD-property dump, has QID Q11129346) ↔ `Kehi Jingū` (21k, clean English article, Jingū wins, NO qid yet). A blind redirect would abandon untranslated source content (which the translation pipeline would otherwise process). **Q for Emma: translate the JP source into the English/Jingū canonical first (route via translation pipeline), or redirect-away anyway (JP → history) per "don't care about damage"?**
  - NOT A MERGE (resolved 2026-06-08): `List of Shikinaisha in Awa Province` is a legit `{{disambiguation}}` page pointing to two different provinces — `(Chiba)` 安房国 and `(Tokushima)` 阿波国. The Q11450714 "overlap" was a resolver false-positive (matched the dab page to one of its targets). Cohort correctly has no QID; partner `(Chiba)` correctly holds Q11450714. Removed from the merge set; dab page left untouched.
