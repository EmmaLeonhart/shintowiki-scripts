...
- [ ] **A4. Stage 4 — LLM remote-queue fallback.** Everything still without an `en` label after A1–A3 goes to the existing claude.ai Sonnet remote routine for translation (5/day drip). Confirm this fires only on the residual set and is correctly last in priority order. Mostly exists — verify ordering and the worklist hand-off (`select_shrines_to_translate.py`). **Known overlap to fix here:** `select_shrines_to_translate.py` still draws from the *full* `shrines_missing_en_label.json` and only dedups against `en_labels_sonnet.txt`, so it can re-translate the ~2305 shrines Stages 1+2 now handle deterministically (424 via `kana_en_labels.txt`, 1881 via `identical_name_en_labels.txt`). Exclude items already covered — simplest: also skip any QID present in `kana_en_labels.txt` or `identical_name_en_labels.txt`.
