...
4. **Wiki changes are lagging indicators — ship and move on; verify later in batch.** A shipped change can take hours (a full cleanup-loop cycle, or many budget-bounded cycles) to manifest on the wiki, and everything on the wiki is fixable/recoverable after the fact. Do NOT stall by waiting to confirm each change works. Instead: ship it, and log anything you couldn't verify in `docs/deferred_verification.md` (what shipped + how to test it). The `monthly-verification-sweep.yml` GitHub Action prepends a task here once a month to walk that doc and actually test each open item.
