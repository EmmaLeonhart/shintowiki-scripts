...
**✅ RESOLVED 2026-05-23 (see DEVLOG):** root cause of the `delete_unused_redirects` failure was a `qppage` casing bug (`Unusedredirects` → `UnusedRedirects`) that started returning `badvalue` ~2026-05-16, plus the `cleanup`→Wikidata workflow coupling that let that Shinto-wiki failure block all Wikidata edits (now decoupled via `needs.cleanup.result != 'cancelled'`). Once the next `cleanup-loop` run is green and the downstream syncs fire, this restart notice + `crashed_session_2026-05-20.md` can be deleted.

**Resume by (historical):** **(0) Clear the bloated queue first — see warning below.** (1) Find the root cause of `cleanup-loop.yml` step 18 (`delete_unused_redirects`) failing as of 03:55 UTC 2026-05-20 — `gh run view <id> --log` on the latest failed run; (2) fix it so syncs downstream can finally fire; (3) once syncs run, the remote queue self-drains. The `remote_queue.py` filter-gap idea (skip files missing the gating category) is a secondary optimization — the primary problem is the broken workflow.
