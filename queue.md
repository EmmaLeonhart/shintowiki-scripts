...
- [ ] **A0. Audit & document the current English-label path.** Map what already exists (`modern-quickstatements/generate_shrines_missing_en_label.py`, `select_shrines_to_translate.py`, `en_labels_sonnet.txt`, `fetch_en_labels_from_wiki.py`, `shinto_miraheze/generate_en_labels_quickstatements.py`) against the target 4-stage design below. Write `docs/english_label_pipeline.md` describing current vs target and where each stage plugs in. No edits to behavior yet — this is the spec the rest of Stage A implements.
