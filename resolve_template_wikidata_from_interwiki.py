"""resolve_template_wikidata_from_interwiki.py
================================================
...
2. For each template page:
   ...
   - Adds {{wikidata link|Q#####}} inside <noinclude> tags
   - If <noinclude> exists: adds after it
   - If no <noinclude>: creates <noinclude>{{wikidata link|Q#####}}</noinclude>
   - If multiple Wikidata items exist, adds all of them
   - If no Wikidata found via interwikis, adds [[Category:templates missing wikidata]]
     inside <noinclude> or creates new <noinclude> tag
...
# Match interwiki links like [[ja:神宮125社]], [[en:Name]], etc.
# The interwiki system automatically applies the same namespace, so we just need lang:title
...
# Match <noinclude> tags
NOINCLUDE_RE = re.compile(r'<noinclude>(.*?)</noinclude>', re.DOTALL)
...
def add_wikidata_to_noinclude(text, qids, found_any_wikidata):
    """Add wikidata link templates and/or missing wikidata category to <noinclude>.

    If <noinclude> exists, adds content after it.
    If no <noinclude>, creates <noinclude>content</noinclude> at the end.
    """

    # Check if <noinclude> exists
    noinclude_match = NOINCLUDE_RE.search(text)

    # Build the content to add
    content_to_add = ""

    # Add wikidata templates if found
    if qids:
        for qid in sorted(set(qids)):  # Remove duplicates and sort
            content_to_add += f"{{{{wikidata link|{qid}}}}}\n"

    # Add missing wikidata category if no wikidata was found
    if not found_any_wikidata:
        content_to_add += "[[Category:templates missing wikidata]]\n"

    # Remove trailing newline for cleaner insertion
    content_to_add = content_to_add.rstrip()

    if noinclude_match:
        # <noinclude> exists - insert content after it
        noinclude_start = noinclude_match.start()
        noinclude_end = noinclude_match.end()
        noinclude_content = noinclude_match.group(1)

        # Add new content to the noinclude section
        new_noinclude = f"<noinclude>{noinclude_content}\n{content_to_add}</noinclude>"

        new_text = text[:noinclude_start] + new_noinclude + text[noinclude_end:]
    else:
        # No <noinclude> - create one at the end
...
def process_template_page(page):
    ...
    if not interwikis:
        ...
        new_text = add_wikidata_to_noinclude(original_text, [], False)
        if new_text != original_text:
            if safe_save(page, new_text, "Bot: add [[Category:templates missing wikidata]] (no interwikis found)"):
    ...
    new_text = add_wikidata_to_noinclude(original_text, all_qids, found_any)
