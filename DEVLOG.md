...
<MIRAHEZE_EMAIL>). Diagnosis first — the bot works on **aelaki**.miraheze.org but not
...
`EmmaBot/2.0 (https://shinto.miraheze.org/wiki/User:EmmaBot; <MIRAHEZE_EMAIL>)`. Replaced
...
`owner=Emma Leonhart <<MIRAHEZE_EMAIL>>` until 2026-06-02, then
auto-switches to `<MIRAHEZE_EMAIL>` (the custom-domain address Emma
