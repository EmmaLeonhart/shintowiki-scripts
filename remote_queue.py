...
BEPPYO_P612_INSTRUCTION = (
    "This file is a WORK ITEM for Wikidata P612 (mother house) on a 別表神社 "
    "(Beppyo shrine). It holds the shrine's Wikidata item, the jawiki article "
    "URL, the full article text, and an empty `<!-- ANSWER: -->` marker.\n\n"
    "Your job: read the article — the 由緒 / 歴史 prose, NOT the infobox, which "
    "never carries this — and identify the shrine this one was BRANCHED FROM "
    "(勧請元 / 分霊元 / 総本社 / 本社). Fill the marker with one of:\n"
    "  `<!-- ANSWER: MOTHER: Q710098 # 石清水八幡宮 -->`  the mother house\n"
    "  `<!-- ANSWER: AUTOCHTHONOUS: <what the article says> -->`  the article "
    "positively describes an in-situ founding with no parent shrine\n"
    "  `<!-- ANSWER: UNCLEAR: <what the article says> -->`  it does not settle it\n\n"
    "Do not confuse the mother house with a shrine merged INTO this one (合祀), "
    "a 摂社/末社 in its grounds, or a shrine this one is the head OF — that is "
    "the reverse direction, and these are head shrines, so it is the likely "
    "error. Where the article names a nearer parent than the network 総本社, "
    "prefer the nearer one. Founding by a person or clan, or 分霊 of a deity "
    "rather than of a named shrine, is not a mother house.\n\n"
    "Give a Q-id you have actually confirmed names that shrine — never one "
    "inferred from a name. If you cannot confirm the item, answer UNCLEAR with "
    "the shrine's name in the text. UNCLEAR is a perfectly good answer and "
    "produces no statement: these are major shrines, and a guessed mother house "
    "is a wrong, highly-visible claim, which is worse than a missing one."
)

NAME_IN_KANA_INSTRUCTION = (
    "This file is a WORK ITEM for Wikidata P1814 (name in kana). It holds a "
    "Shinto shrine's Japanese label, its Wikidata item, the jawiki article URL, "
    "and the article's LEAD, plus an empty `<!-- ANSWER: -->` marker.\n\n"
    "Your job: read the LEAD and write the shrine's reading in MODERN HIRAGANA "
    "into the ANSWER marker, in one of exactly three forms:\n"
    "  `<!-- ANSWER: KANA: ひろしまごこくじんじゃ -->`  the reading, hiragana only\n"
    "  `<!-- ANSWER: KATAKANA: ミシマノオオヤシロ -->`  the lead gives ONLY an "
    "ancient/katakana reading\n"
    "  `<!-- ANSWER: NO_KANA: <reason> -->`            no reading for THIS shrine "
    "in the lead\n\n"
    "The reading is normally the parenthetical right after the bolded name in the "
    "first sentence. Take the reading of the SHRINE NAME ITSELF — not a 通称 "
    "(common name), not a 旧称 (former name), not the reading of the town it "
    "stands in, and not a second shrine the lead happens to mention. Drop "
    "interpuncts and spaces. Do NOT romanize, do NOT translate, do NOT invent a "
    "reading the lead does not give — NO_KANA is the correct answer when it is "
    "absent.\n\n"
    "Why KATAKANA is a separate answer and not just a reading: a katakana "
    "reading is the signature of an old import error that a separate cleanup is "
...
DESCRIPTION_ENRICHMENT_INSTRUCTION = (
    ...
    "Wikidata register (no final period; capitalize only a proper noun, which "
    "for these means the leading 'Shinto' - measured 2026-08-05, the corpus is "
    "11,487 'Shinto ...' to 1 'shinto ...'); leave a line empty "
...
MISSING_LEAD_INSTRUCTION = (
    "This page's only lead is the autogenerated stub sentence "
    "(`{{nihongo|'''Name'''|漢字}} is a [[shinto shrine]] in the [[Engishiki "
    "Jinmyōchō]]. It is located in [[X Province]].`). The 905-page merge left it "
    "in place because there was no imported jawiki lead to merge it into — "
    "blanking it would have destroyed the article's lead entirely. "
    "Write a real lead from the corresponding Japanese Wikipedia article: find "
    "its title from the page's `{{wikidata link|QID|ja|TITLE}}` or `[[ja:TITLE]]`, "
    "read that article, and REPLACE the stub line with a proper opening "
    "paragraph in the house style — `{{nihongo|'''Name'''|漢字}}` followed by a "
    "sentence saying what the shrine is and where it is, then whatever the "
    "Japanese lead says about its enshrined kami, founding, and rank. Keep the "
    "stub's factual claims (Engishiki Jinmyōchō listing, province) somewhere in "
    "the paragraph — they are correct. Do not touch anything below the lead, do "
    "not remove [[Category:Formerly autogenerated pages]], and do not invent "
    "facts the Japanese article does not state."
)

# The stub as the WHOLE line. Anchoring at both ends, and forbidding the groups
# from crossing a ''' or }} boundary, is load-bearing: `finish_japanese_content_
# merge.STUB_LEAD` uses lazy `.+?` groups, and on a *merged* page (where the stub
# sentence is appended verbatim to the end of the imported lead) those groups
# happily span the whole line and match. Reusing it here reported 892 pages
# needing leads instead of 149 — it was matching the 755 already-merged ones.
_PURE_STUB = re.compile(
    r"^\{\{nihongo\|'''[^']+'''\|[^}]+\}\} is a \[\[shinto shrine\]\] in the "
    r"\[\[Engishiki Jinmyōchō\]\]\.( It is located in \[\[[^\]]+\]\]\.)?\s*$"
)
# The imported jawiki lead: `'''Name''' (kana) is …`
_IMPORTED_LEAD = re.compile(r"^'''.+?''' \([^)]*\)\s+is .*$")
_ANY_HEADING = re.compile(r"^==+.*==+\s*$")
_FORMERLY_AUTOGEN = "[[Category:Formerly autogenerated pages]]"


def _needs_hand_written_lead(path: Path) -> bool:
    """git_synced/ pages whose only lead is the autogenerated stub.

    Stateful purely on the file's shape: once the worker replaces the stub line
    with a real lead, `_PURE_STUB` stops matching and the item leaves the queue.
    No cursor, no category to strip.
    """
    text = path.read_text(encoding="utf-8", errors="replace")
    if _FORMERLY_AUTOGEN not in text:
        return False
    lines = text.split("\n")
    if not any(_PURE_STUB.match(ln) for ln in lines):
        return False
    end = next((i for i, ln in enumerate(lines) if _ANY_HEADING.match(ln)), len(lines))
    return not any(_IMPORTED_LEAD.match(lines[i]) for i in range(end))
...
# ⛔ BOTH WIKIS ARE DEAD, SO THE WIKI-BOUND HALF OF THIS QUEUE IS OFF (Emma, 2026-09-15)
#
#     "I think that almost all of the non-labelling grunge is shit that is just
#      straight up 100% blocked by the wiki being dead. Both wikis are dead so
#      forget about them."
#
# Every gated category below drains the SAME way: the worker edits the mirrored file
# and removes its gating category, and a sync pushes the page and deletes the local
# copy. With shinto.miraheze.org behind a Cloudflare managed challenge from the
# runners and Fandom out too, that push cannot happen — so an answered item is an
# edit that sits in the repo, reaches no wiki, and has consumed one of the five slots
# for that day.
#
# ⭐ THIS IS NOT A RATE CHANGE. The routine still takes 5 random items/day and
# CLAUDE.md's rule that the slowness is deliberate is untouched. What changes is
# WHICH five: all of them now fall on work whose destination is Wikidata via the
# QuickStatements collectors, which is reachable. Before this, 1,124 of 2,091 items —
# 54% — were wiki-bound, so more than half of every day's budget went on edits that
# could not land.
#
# TO TURN THEM BACK ON: set WIKI_REACHABLE = True. Nothing else. The sections, their
# instructions and their filters are all still here and still tested; they are
# SKIPPED, not deleted, because the wikis coming back is the expected end state.
...
def build_queue() -> list[dict]:
    ...
    # Queue item A0: read the jawiki lead and give the shrine's MODERN HIRAGANA
    # reading for P1814 (see build_name_in_kana_queue.py); collect_name_in_kana.py
    # gates the answer to hiragana and emits the QuickStatement.
    items.extend(_build_section("name_in_kana", NAME_IN_KANA_INSTRUCTION))
    # Queue item A0b: read the jawiki article and name the shrine's mother house
    # (see build_beppyo_p612_queue.py); collect_beppyo_p612.py emits the single
    # P612 + P1013=Q195793 statement.
    items.extend(_build_section("beppyo_p612", BEPPYO_P612_INSTRUCTION))
    # Emma 2026-07-09: the pages the 905-page merge left with only an autogenerated
    # stub lead get a hand-written one, researched from their jawiki article.
    # Wiki-bound like the block at the top — a hand-written lead is worth nothing
    # until sync_git_synced_pages.py can push it to Miraheze.
    if WIKI_REACHABLE:
        items.extend(
            _build_section(
                "git_synced",
                MISSING_LEAD_INSTRUCTION,
                filter_fn=_needs_hand_written_lead,
            )
        )
...
def main() -> int:
    # Rebound here, not at import: doing it at module scope replaced pytest's
    # captured stdout with a wrapper around the *closed* underlying buffer, and
    # every test in this file died with "I/O operation on closed file" before it
    # could run.
    ...
    # Count what actually went in rather than naming three sections by hand: the
    # hardcoded list had been wrong since the labeling queues were added, and it
    # would be wrong again the moment WIKI_REACHABLE flips.
    by_cat = collections.Counter(i["category"] for i in items)
    mix = ", ".join(f"{c} {n}" for c, n in by_cat.most_common())
    print(
        f"Wrote {out_path.relative_to(REPO_ROOT) if out_path.is_absolute() else out_path} "
        f"with {len(items)} items ({mix})."
    )
    if not WIKI_REACHABLE:
        print("  WIKI_REACHABLE=False — wiki-bound sections skipped (both wikis down).")
    return 0
