"""add_wikidata_from_id_interwiki_category_ms.py
================================================
Add wikidata link templates from Malay Wikipedia interwiki links.
Reads pages from a specific category instead of pages.txt.
================================================

This script:
1. Gets all pages from [[Category:Islamic Calendar Days]]
2. Finds [[ms:...]] interwiki links on each page
3. For each Malay Wikipedia article found, queries Wikidata for the QID
4. Adds {{wikidata link|QID}} template to the bottom of the page

Only modifies pages that have Malay interwiki links that exist on Wikidata.
Only processes pages without existing wikidata links.
"""
...
# ─── REGEX PATTERNS ─────────────────────────────────────────
# Match [[ms:...]] interwiki links
MS_INTERWIKI_RE = re.compile(r'\[\[ms:([^\]]+)\]\]')
...
def extract_ms_interwiki(text):
    """Extract Malay Wikipedia article title from [[ms:...]]."""
    match = MS_INTERWIKI_RE.search(text)
    if match:
        return match.group(1)
    return None


def get_qid_from_wikipedia(wiki_title, language="ms"):
...
def process_page(page):
    """Process a single page to add wikidata link from Malay interwiki."""
    try:
    ...
    # Extract Malay interwiki link
    ms_article = extract_ms_interwiki(original_text)
    if not ms_article:
        # No Malay interwiki found
        return False

    # Query Wikidata for the QID
    qid = get_qid_from_wikipedia(ms_article, "ms")
    if not qid:
        # No Wikidata entry found for this Malay Wikipedia article
        print(f"   • [[{page.name}]] has [[ms:{ms_article}]] but no Wikidata entry found")
        return False

    # At this point we know: no existing wikidata links, has Malay interwiki, and QID found
    qid_upper = qid.upper()
    ...
    if new_text != original_text:
        if safe_save(page, new_text, f"Bot: add wikidata link from Malay interwiki ({qid_upper})"):
            print(f"   • added {{{{wikidata link|{qid_upper}}}}} to [[{page.name}]] from [[ms:{ms_article}]]")
            return True
