#!/usr/bin/env python3
"""
fa_interwiki_wikidata_sync_bot.py  –  append Wikidata-derived interwiki blocks
-------------------------------------------------------------------------
For every page supplied in *pages.txt* this bot:
  1. Scans the pageʼs wikitext for the **first** {{fa:…}}, {{de:…}},
  ...
  3. Writes four comment-delimited blocks **at the bottom** of the page,
     always in the fixed order ↓
        ...
        <!--fawiki derived wikidata interwikis-->
        …links…
...
LANG_SITES = {
    "fa": mwclient.Site("fa.wikipedia.org", path="/w/"),
...
# ─── REGEXES ──────────────────────────────────────────────────────────
IW_RE = re.compile(r"\[\[\s*(fa|de|zh|en)\s*:\s*([^\]|]+)")
