#!/usr/bin/env python3
"""
patch_ill_english_labels_v3.py  –  ShintoWiki ILL-template fixer
────────────────────────────────────────────────────────────────
...
# ─────────────────── credentials ─────────────────────────────────────────
WD_USER = "EmmaBot@EmmaBotGeniTestBot"
...
def ensure_param_1(tpl: mwp.nodes.template.Template, value: str) -> bool:
    """
    Add or replace |1=.  Returns True if text changed.
    """
    if tpl.has("1"):
        current = str(tpl.get("1").value).strip()
...
def process_page(title: str, dry: bool):
    ...
    for tpl in code.filter_templates(recursive=True):
        ...
        if "en" in labels:
            ...
            if ensure_param_1(tpl, wd_en):
                logging.info("· patched |1=%s", wd_en)
                changed = True
        else:
