...
# -*- coding: utf-8 -*-
"""
Wiktionary bot to add Wikidata lexeme templates to English entries.

For each English part of speech section, queries Wikidata for a matching lexeme
...
def query_wikidata_lexeme(lemma, language_qid='Q1860', pos_qid=None):
    """
    Query Wikidata for a lexeme with matching lemma, language, and part of speech.

    Args:
        lemma: The lemma text to search for
        language_qid: Wikidata QID for the language (default Q1860 = English)
...
def parse_english_section(wikitext):
    """
    Parse the wikitext to find English section and its part of speech subsections.

    Returns:
        List of tuples: (pos_header, start_pos, end_pos)
    """
    # Find the English L2 section
    english_pattern = r'==\s*English\s*=='
    english_match = re.search(english_pattern, wikitext)
    ...
    next_l2_pattern = r'\n==\s*[^=]+\s*=='
    next_l2_match = re.search(next_l2_pattern, wikitext[english_start:])

    if next_l2_match:
        english_end = english_start + next_l2_match.start()
    else:
        english_end = len(wikitext)

    english_text = wikitext[english_start:english_end]

    # Find all L3 part of speech headers in English section
    pos_sections = []
    pos_pattern = r'===\s*(' + '|'.join(re.escape(pos) for pos in POS_MAP.keys()) + r')\s*==='

    for match in re.finditer(pos_pattern, english_text):
        pos_header = match.group(1)
        pos_start = english_start + match.start()
        pos_end_match = re.search(r'\n===', english_text[match.end():])

        if pos_end_match:
            pos_end = english_start + match.end() + pos_end_match.start()
        else:
            pos_end = english_end
...
def process_page(site, page_title):
    ...
    # Parse English section and find POS subsections
    pos_sections = parse_english_section(wikitext)

    if not pos_sections:
        print(f"  No English section or recognized POS found")
        return False
    ...
    if edits_made:
        ...
        if len(edits_made) == 1:
            ...
            summary = (f'Early testing of a script as per [[Wiktionary:Bots#Process]]. '
                      ...
                      f'as per English, {pos_header.lower()}, title:"{page_title}" combination')
        else:
...
def main():
    ...
    print("Wiktionary Lexeme Bot")
    print("=" * 50)

    # Get page titles from command-line arguments
    if len(sys.argv) < 2:
        print("\nUsage: python wiktionary_lexeme_bot.py <page1> [page2] [page3] ...")
        print("Example: python wiktionary_lexeme_bot.py hydrogen helium oxygen")
        sys.exit(1)
