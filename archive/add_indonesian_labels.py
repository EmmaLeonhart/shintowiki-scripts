#!/usr/bin/env python3
"""
add_Indonesian_labels.py
────────────────────
For every Q845945 instance that has an English label like "X Shrine" but no Indonesian label,
add a Indonesian label of the form "X-schrijn".

USAGE
─────
  python add_Indonesian_labels.py            # live
  python add_Indonesian_labels.py --dry      # preview only
  python add_Indonesian_labels.py --verbose  # chatty
"""
...
BOT_PASS = "030akvvhf8b3f6fg7mpt85fo8rvp6d58"
WD_USER = BOT_USER
WD_PASS = BOT_PASS
...
HEADERS = {
    "User-Agent": "ImmanuelleIndonesianLabelBot/0.1 (https://wikidata.org/wiki/User:Immanuelle; mailto:immanuelleleonhart@gmail.com)"
}
...
def add_Indonesian_label(qid, nl_label, verbose, dry):
    if dry and verbose:
    ...
    r = wd_post({
        ...
        "language": "id",
        "value": nl_label,
        "summary": "Bot: add Indonesian label from English"
    })
...
def query_items():
    query = """SELECT ?item ?enLabel WHERE {
      ...
      OPTIONAL { ?item rdfs:label ?nlLabel. FILTER(LANG(?nlLabel) = "id") }
...
def main(dry=False, verbose=False):
    ...
    for qid, en_label in items:
        ...
        result = add_Indonesian_label(qid, nl_label, verbose, dry)
        if result == "added":
            added += 1
        else:
            skipped += 1
    ...
    print(f"\n✅ {added} Indonesian label(s) {action}, {skipped} skipped.")
