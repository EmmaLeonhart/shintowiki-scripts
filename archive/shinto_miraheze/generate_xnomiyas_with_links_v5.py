...
VERSION 4: FIXED - Ensures all locations and rankings are properly {{ill|}} linked

This version creates individual pages for each province with full linking:
[[User:Immanuelle/X-no-Miyas/Province Name]]

Table columns:
1. Region (province's P361 - MULTIPLE comma-separated and linked)
2. Province (linked)
3. Shrine (linked)
4. Location (shrine's P131 - MULTIPLE comma-separated and linked)
5. Engishiki Funding Category (placeholder - from Shikinaisha list)
6. Engishiki Ranking (placeholder - from Shikinaisha list)
7. Modern Ranking (shrine's P13723 - LINKED)
8. Beppyo Shrine status
9. Coordinates (shrine's P625)

ALL QID references are converted to {{ill|}} links with proper formatting.
"""
...
def create_ill_link(qid, entity_data):
    ...
    Returns the QID itself if no entity data available.
    """
...
def get_engishiki_funding(claims, all_entities):
    """Get Engishiki funding category (Kanpei or Kokuhei) from P31."""
    ...
    for p31_claim in claims['P31']:
        ...
        if p31_qid == KANPEI_QID:
            ...
            return create_ill_link(KANPEI_QID, kanpei_entity) if kanpei_entity else "Kanpei"
        elif p31_qid == KOKUHEI_QID:
            kokuhei_entity = all_entities.get(KOKUHEI_QID)
            return create_ill_link(KOKUHEI_QID, kokuhei_entity) if kokuhei_entity else "Kokuhei"
    return ""
...
def create_table_for_role(role_qid, shrine_qids, province_qid, all_entities):
    ...
    for shrine_qid in shrine_qids:
        ...
        # Get all locations (P131 - can be multiple) and CREATE ILL LINKS FOR EACH
        located_in_qids = get_property_values(claims, 'P131')
        located_in_links = [create_ill_link(qid, all_entities.get(qid)) for qid in located_in_qids]
        located_in_text = ", ".join(located_in_links) if located_in_links else ""

        # Get ranking and CREATE ILL LINK
        ...
        # Get Engishiki funding and rank
...
def main():
    """Main execution."""
    print("="*70)
    print("X-NO-MIYAS INDIVIDUAL PAGE GENERATOR V4 (FIXED LINKING)")
    print("="*70)
    print()
    ...
    try:
        ...
        while current_qid and province_num <= 68:
            ...
            if parts_by_role:
                ...
                # Also fetch province's regions and Engishiki categories
                ...
                # Always fetch Engishiki QIDs for proper linking
                ...
                # Pre-fetch shrine entities to extract location and ranking references
                ...
                # Now fetch the roles and related entities (locations, rankings)
