#!/usr/bin/env python3
"""generate_shikinaisha_pages_v15.py
================================================
Generate standardized shrine pages for Wikidata-generated Shikinaisha entries
V15: Fix ILL template format (restore proper form), fix commons category
================================================
...
4. Generates standardized page content with:
   ...
   - ILL templates with PROPER format: {{ill|ENGLISH_LABEL|lt=ENGLISH_LABEL|en|ENGLISH|ja|JAPANESE|...}}
   - P625 (Coordinate Location) formatted with {{coord|lat|lon}} template
   - Source references <ref>SOURCE_WITH_URL</ref> appended after property bullets
   ...
   - Interwiki categories based on sitelinks:
     * {{Expand Japanese|JAWIKI_TITLE}} at top + category if jawiki exists
     * {{Expand Chinese|ZHWIKI_TITLE}} if zhwiki exists and NO jawiki
     * Category for enwiki/simplewiki: "Autogenerated pages with simplewiki or enwiki interwikis, possibly accidentally overwritten"
     * Category for jawiki: "Autogenerated pages with jawiki interwikis, possibly accidentally overwritten"
...
def format_wikidata_link(entity, qid):
    """Format a Wikidata entity as an ILL (interlanguage link) template using PROPER format."""
    if not entity:
        return f"{{{{ill|{qid}|lt={qid}|WD={qid}}}}}"
    ...
    # Build the template with proper positional parameters
    template = f"{{{{ill|{english_label}"

    # Add lt parameter with English label
    template += f"|lt={english_label}"

    # Add language-specific labels with positional parameters (not keyword parameters)
    # Order: en, ja, de, zh, fr, es, it, pt, ru
    languages = ['en', 'ja', 'de', 'zh', 'fr', 'es', 'it', 'pt', 'ru']

    for lang in languages:
        label = get_label(entity, lang)
        if label:
            template += f"|{lang}|{label}"

    # Add WD parameter at the end
    template += f"|WD={qid}}}}}"
...
def get_source_reference(claim):
    ...
    for ref in references:
        ...
        # First, check for reference URL (P854)
        url = None
        ...
        # Prefer URL, fall back to source label
        if url:
            if source_label:
...
def format_page_content(page_name, qid, entity):
    ...
    final_content = "<!--generated by generate_shikinaisha_pages_v15.py-->\n" + "\n".join(content_parts)
...
def main():
    ...
    print("Generating standardized Shikinaisha pages (V15 - Fix ILL format, commons category)\n")
    print("=" * 60)
    ...
    for idx, page in enumerate(members, 1):
        try:
            ...
            # Save the page
            try:
                page.edit(new_content, summary="Bot: Fix ILL template format (V15 - restore proper form) and commons category - revert V14 ILL damage")
                processed_count += 1
                print(f" ... ✓ Updated")
