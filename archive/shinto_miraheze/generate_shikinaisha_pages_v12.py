#!/usr/bin/env python3
"""generate_shikinaisha_pages_v12.py
================================================
Generate standardized shrine pages for Wikidata-generated Shikinaisha entries
V12: Add lt= parameter to ILL templates, omit P1448, proper P625 coordinate formatting, source refs
================================================

This script:
1. Waits 2 hours before starting to reduce server load
...
4. Generates standardized page content with:
   ...
   - ILL templates with proper lt= parameter set to English label
   - P625 (Coordinate Location) formatted with {{coord|lat|lon}} template
   - Source references <ref>SOURCE</ref> appended after property bullets
...
PROPERTIES_TO_OMIT = ['P1448']  # Don't create sections for these
...
def wait_before_start():
    """Wait 2 hours before starting the main process."""
    print("="*70)
    print("V12 STARTUP DELAY (Reduce Server Load)")
    print("="*70)
    print(f"\nThis version includes a 2-hour startup delay.")
...
def format_wikidata_link(entity, qid, link_text=None):
    """Format a wikidata link using ILL syntax with lt= parameter.

    The lt= parameter is set to the English label to ensure proper display text.
    """
...
def get_source_reference(claim):
    """Extract source reference from claim references if available."""
    references = claim.get('references', [])
    if not references:
        return None

    # Look for reference URL or retrieved sources
    for ref in references:
        ref_snaks = ref.get('snaks', {})

        # Check for stated in (P248)
...
def format_claim_value(claim, entity, property_id=None):
    ...
    Special handling:
    - P13677: Kokugakuin Digital Museum entry ID
    - P625: Coordinate location with {{coord|}} template
    - Source references appended as <ref>SOURCE</ref>
    """
    mainsnak = claim.get('mainsnak', {})
    datavalue = mainsnak.get('datavalue', {})
    ...
    source = get_source_reference(claim)
...
def format_page_content(page_name, qid, entity):
    ...
    final_content = "<!--generated by generate_shikinaisha_pages_v12.py-->\n" + "\n".join(content_parts)
...
def main():
    ...
    print("Generating standardized Shikinaisha pages (V12 - ILL lt= param, P625 coords, omit P1448, source refs)\n")
    print("=" * 60)
    ...
    for idx, page in enumerate(members, 1):
        try:
            ...
            # Save the page
            try:
                page.edit(new_content, summary="Bot: Generate standardized Shikinaisha page (V12 - ILL lt= parameter, P625 coords, P1448 omitted, source references)")
                processed_count += 1
                print(f" ... ✓ Updated")
