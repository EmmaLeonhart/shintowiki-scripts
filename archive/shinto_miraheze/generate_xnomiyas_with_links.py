...
def get_label(qid, label_cache):
    ...
    if qid in label_cache and 'labels' in label_cache[qid]:
        labels = label_cache[qid]['labels']
        if 'en' in labels:
...
def create_table_for_role(role_qid, shrine_qids, province_qid, province_label, all_entities):
    ...
    for shrine_qid in shrine_qids:
        ...
        # Create province link
        province_link = create_ill_link(province_qid, province_entity) if province_entity else province_label
...
def generate_and_post_province_page(province_qid, province_list_qid, all_entities, site):
    ...
    province_link = create_ill_link(province_qid, province_entity)
    ...
    for role_qid in sorted(parts_by_role.keys(), key=lambda x: (x is None, x)):
        ...
        table = create_table_for_role(role_qid, shrine_qids, province_qid, province_label, all_entities)
        page_content += table + "\n"
...
def main():
    """Main execution."""
    print("="*70)
    print("X-NO-MIYAS INDIVIDUAL PAGE GENERATOR (WITH INTERLANGUAGE LINKS)")
    print("="*70)
    print()
    ...
    try:
        ...
        while current_qid and province_num <= 68:
            ...
            if parts_by_role:
                ...
                # Fetch all shrine and role entities with labels and sitelinks
