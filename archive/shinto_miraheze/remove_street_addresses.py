#!/usr/bin/env python3
"""
Remove all street address (P6375) section headings from pages in
...
def main():
    """Main execution."""
    print("="*70)
    print("REMOVE STREET ADDRESS (P6375) HEADINGS")
    print("="*70)
    print()
        ...
        try:
            for page in site.api('query', list='categorymembers', cmtitle=f'Category:{category_name}', cmlimit='max')['query']['categorymembers']:
                ...
                try:
                    ...
                    # Look for P6375 heading and its content - remove until next heading or end
                    pattern = r'==\s*street address \(P6375\)\s*==\s*\n(?:(?!==)[\s\S])*'

                    new_text = re.sub(pattern, '', page_text)

                    if new_text != page_text:
                        # Changes were made
                        page_obj.edit(new_text, summary="Remove street address (P6375) section heading")
                        print(f"[MODIFIED]")
