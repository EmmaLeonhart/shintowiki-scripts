"""zh_full_replace_bot.py
========================
For every **local page title** in *pages.txt* this bot:

1. Loads the page and extracts the first `[[zh:…]]` inter‑wiki link.
2. Imports the **full history** of that zh‑wiki page (transwiki → XML
   fallback).
3. Builds a new top‑revision wikitext:
      * original content **plus** a
        `{{translated page|zh|<zh_title>|version=<rev_id>|comment=…}}`
        tag appenzhd at the bottom.
4. **deletes** the local page (archives its revisions).
5. **Moves** the freshly‑imported zh‑page onto the local title
   (no redirect).
...
8. Cleans up the leftover redirect at the old zh title.

Rights required: `import`, `importupload`, `move`, `delete`, `undelete`.

Usage: edit *CONFIG*, list local titles in *pages.txt*, `python zh_full_replace_bot.py`.
"""
...
FAIL_CAT  = "[[Category:zh import failed]]"  # category adzhd on failure
...
zh_LINK_RE = re.compile(r"\[\[\s*zh:([^|\]]+)", re.I)
...
def load_titles() -> list[str]:
    if not os.path.exists(PAGES_TXT):
        ...
        print(f"Created empty {PAGES_TXT}; add local titles and re‑zhn.")
        sys.exit()
    with open(PAGES_TXT, "r", encoding="utf-8") as fh:
        return [ln.strip() for ln in fh if ln.strip() and not ln.startswith('#')]

# ─── zhPANESE WIKI HELPERS ─────────────────────────────────────────

def zh_export_xml(title: str) -> bytes:
    url = "https://zh.wikipedia.org/wiki/Special:Export/" + urllib.parse.quote(title, safe="")
    r = requests.get(url, params={"history": "1"}, timeout=90)
    r.raise_for_status()
    return r.content


def zh_last_rev_id(title: str) -> str | None:
    params = {"action": "query", "prop": "revisions", "rvprop": "ids", "rvlimit": 1,
              "titles": title, "format": "json"}
    data = requests.get("https://zh.wikipedia.org/w/api.php", params=params, timeout=30).json()
...
def import_history(zh_title: str, rev_id: str, token: str) -> bool:
    try:
        site.api("import", token=token, interwikisource="zhwiki", interwikipage=zh_title,
                 fullhistory=1,
                 summary=f"Bot: import full history from zh:{zh_title} up to rev {rev_id}")
...
def tag_failure(page):
    ...
    try:
        page.save(page.text() + "\n" + FAIL_CAT + "\n",
                  summary="Bot: mark zh import failure")
        print("    ! adzhd failure category")
...
def merge_by_replace(local: str, zh_title: str, new_text: str, token: str) -> bool:
    local_page = site.pages[local]
    zh_page    = site.pages[zh_title]


    if not zh_page.exists:
        print("        ! imported zh page missing – cannot merge")
        return False
    ...
    try:
        ...
        # move zh → local (no redirect)
        print(f"        → move [[{zh_title}]] → [[{local}]] (no redirect)")
        zh_page.move(local, reason="Bot: merge import", no_redirect=True, move_subpages=False)
        ...
        if site.pages[zh_title].exists:
            site.pages[zh_title].delete(reason="Bot: cleanup redirect", watch=False)
...
def process_page(local_title: str):
    ...
    m = zh_LINK_RE.search(page.text())
    if not m:
        print("    ! no zh link – skipped")
        return
    zh_title = m.group(1).strip()


    from urllib.parse import unquote
    zh_title = unquote(zh_title).replace('_', ' ')

    print(f"    → zh:{zh_title}")

    rev_id = zh_last_rev_id(zh_title)
    if not rev_id:
        print("    ! could not fetch zh revID – tagging failure")
        tag_failure(page)
        return

    # prepare final content with translated tag
    tag = f"\n{{{{translated page|zh|{zh_title}|version={rev_id}|comment=Imported full zh history}}}}\n"
    final_text = page.text() + tag

    token = site.get_token("csrf")
    if not import_history(zh_title, rev_id, token):
        return

    if merge_by_replace(local_title, zh_title, final_text, token):
