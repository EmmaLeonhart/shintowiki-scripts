...
USAGE
─────
  python add_turkish_labels.py            # live
  python add_turkish_labels.py --dry      # preview only
  python add_turkish_labels.py --verbose  # chatty
"""
...
def query_items():
    query = """SELECT ?item ?enLabel WHERE {
      ...
      OPTIONAL { ?item rdfs:label ?nlLabel. FILTER(LANG(?nlLabel) = "tr") }
