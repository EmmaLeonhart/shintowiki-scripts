...
def main():
    ...
    print(f"Processing pages from the beginning...\n")
    ...
    for idx, page in enumerate(members, 1):
        try:
            ...
            # Skip if already has interwikis comment
            if has_interwiki_comment(text):
                print(f"{idx}. [[{page_name}]] ({qid}) ... • Already has interwikis")
                skipped_count += 1
                continue
            ...
            if not interwikis:
                print(f" ... • No interwikis found", flush=True)
                continue
            ...
            if safe_save(page, new_text, "Bot: add interwikis from Wikidata"):
                processed_count += 1
                print(f" ... ✓ Added {len(interwikis)} interwikis", flush=True)
            else:
