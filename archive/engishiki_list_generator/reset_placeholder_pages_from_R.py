#!/usr/bin/env python3
"""
reset_placeholder_pages_from_R.py

Overwrites every page in [[Category:Wikidata generated shikinaisha pages]]
starting from letter R onwards with a placeholder message.
"""
...
if __name__ == "__main__":
    ...
    # Filter to only pages starting with R or later
    filtered_members = [m for m in members if m[0] >= 'R']
    print(f"[INFO] Processing {len(filtered_members)} pages starting from R onwards")

    print("[INFO] Starting reset from letter R...")
    count = 0
    for title in filtered_members:
        if overwrite_page(title, csrf):
            count += 1

    print(f"\n[DONE] Reset {count}/{len(filtered_members)} pages (starting from R)")
