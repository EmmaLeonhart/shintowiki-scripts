...
# Resume point
RESUME_FROM = "Gongen Shrine (Ujizakura Shrine precincts)"
    ...
    if page.namespace == 0:  # Main namespace only
        ...
        if not found_resume_point:
            if page.name == RESUME_FROM:
                ...
                print(f"Found resume point: {RESUME_FROM}", flush=True)
            else:
                continue  # Skip until we find it

        pages_to_check.append(page)

print(f"Found {len(pages_to_check)} pages to check (starting from {RESUME_FROM})", flush=True)
