...
for page in all_redirects:
    ...
    # Skip QID redirects (titles starting with Q)
    if page.name.startswith('Q'):
        skipped_qid_count += 1
        print(f"\n[{redirect_count}] {page.name} - SKIPPING (QID redirect)", flush=True)
        continue
...
print(f"\n\nDone! Processed {redirect_count} redirects, skipped {skipped_qid_count} QID redirects, removed categories from {edit_count} pages.", flush=True)
